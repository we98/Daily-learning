...
- ArrayList代码分析：
  ...
  - 扩容时函数调用
    ...
    - ensureCapacity(minCapacity)，这个函数会取出minCapacity与10的最大值，调用ensureExplicitCapacity(minCapacity)，作用是得到最小需要的容量
    - ensureExplicitCapacity(minCapacity)，这个函数会检查minCapacity与数组length的大小，如果minCapacity比较大，则增长数组，调用grow(minCapacity)，作用是判断最小需要容量与数组长度的关系，再确定是否调用扩容函数
